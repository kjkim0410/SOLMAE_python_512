...
def Solmae_fft(f):
    n = len(f)
    f_fft = fft(f)
    return [f_fft[i].real for i in range(n//2)]+[f_fft[i].imag for i in range(n//2)]

def Solmae_ifft(solmae_f_fft):
    n = len(solmae_f_fft)
    f_fft = [complex(solmae_f_fft[i], solmae_f_fft[i+n//2]) for i in range(n//2)]
    f_fft += [complex(solmae_f_fft[i], -solmae_f_fft[i+n//2]) for i in range(n//2)]
    return ifft(f_fft)

def cut_half_fft(f_fft):
    n = len(f_fft)
    return [f_fft[i].real for i in range(n//2)] + [f_fft[i].imag for i in range(n//2)]
